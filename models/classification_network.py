...
def MLP(channels, batch_norm=True):
    return Seq(*[Seq(Lin(channels[i - 1], channels[i]),BN(channels[i]) if batch_norm else Identity(), ReLU())
...
class ClassificationNetwork(torch.nn.Module):
    ...
    def __init__(self,class_nb,pretrained_class_net,use_text) -> None:
        super().__init__()
        self.param = torch.nn.Parameter(torch.ones(1,))

        inFeat = 3 if use_text else 0

        self.net = Net(inFeat, class_nb,dim_model=[32, 64, 128, 256, 512], k=16)

        if pretrained_class_net:
            keysToIgnore = ["mlp_output.4.weight","mlp_output.4.bias","mlp_input.0.0.weight"]

            dic = torch.load("best_model_110.pt",map_location="cpu")
            newDic = {}
            keys_to_keep = []
            for key in dic.keys():
            ...
            missing,excess = self.net.load_state_dict(newDic,strict=False)

            if len(excess) > 0:
                raise ValueError("Excess keys",excess)
            
            if len(missing) > 0:
                if not set(missing) == set(keysToIgnore):
                    raise ValueError("Missing keys",missing)

    def forward(self,pos,x=None):
        batch = torch.arange(pos.shape[0], device=pos.device).unsqueeze(1).expand(pos.shape[0], pos.shape[1]).reshape(-1)
        pos = pos.view(pos.shape[0]*pos.shape[1],-1)
        if not x is None:
           x = x.view(x.shape[0]*x.shape[1],-1) 
        out = self.net(x=x,pos=pos,batch=batch)
        return out
...
def main():
    ...
    classNet = ClassificationNetwork(200,True)
